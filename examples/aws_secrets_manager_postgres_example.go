...
func secrets_manager_postgres() {
	...
	connStr := fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s plugins=%s secretsManagerSecretId=%s secretsManagerRegion=%s secretsManagerEndpoint=%s",
		host, port, user, password, dbName, plugins, secretsManagerSecretId, secretsManagerRegion, secretsManagerEndpoint,
	)

	// alternative connection string for pgx
	// connStr := fmt.Sprintf("postgresql://%s:%s@%s:%s/%s?plugins=%s&secretsManagerSecretId=%s&secretsManagerRegion=%s&secretsManagerEndpoint=%s",
