...
type PluginFactoryFunc func() ConnectionPluginFactory
...
func (builder *ConnectionPluginChainBuilder) GetPlugins(
	...
	props map[string]any) ([]ConnectionPlugin, error) {
	var plugins []ConnectionPlugin
	var pluginFactoryFuncWeights []PluginFactoryFuncWeight
	usingDefault := false

	pluginCodes := PLUGINS.Get(props).(string)
	if pluginCodes == DEFAULT_PLUGINS {
		usingDefault = true
	}
	...
	for _, pluginCode := range pluginCodes {
		factoryFunc, ok := pluginFactoryFuncByCode[string(pluginCode)]
		if !ok {
			return nil, &AwsWrapperError{
				message:   GetMessage("ConnectionPluginManager.unknownPluginCode", pluginCode),
				errorType: GenericAwsWrapperErrorType,
			}
		}
		if lastWeight == WEIGHT_RELATIVE_TO_PRIOR_PLUGIN {
			lastWeight++
		} else {
			lastWeight = pluginWeightByCode[string(pluginCode)]
		}
		pluginFactoryFuncWeights = append(pluginFactoryFuncWeights, PluginFactoryFuncWeight{factoryFunc, lastWeight})
	}

	autoSort := AUTO_SORT_PLUGIN_ORDER.Get(props).(bool)
	pluginsSorted := false
	...
	plugins = append(plugins, &DefaultPlugin{
		PluginService:       *pluginService,
		PluginManager:       pluginManager,
		DefaultConnProvider: pluginManager.defaultConnProvider,
		ConnProviderManager: pluginManager.connProviderManager,
	})
	if pluginsSorted {
		slog.Info(fmt.Sprintf("Plugins order has been rearranged. The following order is in effect: '%v'.", getFactoryOrder(plugins)))
	}

	return plugins, nil
}

func getFactoryOrder(pluginFactoryFuncWeights []ConnectionPlugin) string {
