...
func NewXRayTelemetryContext(name string, traceLevel TelemetryTraceLevel, ctx context.Context) (*XRayTelemetryContext, context.Context) {
	...
	if parentSegment == nil && effectiveTraceLevel == NESTED {
	...
	case TOP_LEVEL:
		...
		if parentSegment != nil {
			xrayCtx.SetAttribute(PARENT_TRACE_ANNOTATION, parentSegment.ParentID)
			xrayCtx.SetAttribute(PARENT_SPAN_ANNOTATION, parentSegment.ID)
		}
		xrayCtx.SetAttribute(TRACE_NAME_ANNOTATION, name)
...
func XRayPostCopy(telemetryCtx XRayTelemetryContext, traceLevel TelemetryTraceLevel) {
...
func (x *XRayTelemetryContext) Clone(traceLevel TelemetryTraceLevel) *XRayTelemetryContext {
	copyCtx, _ := NewXRayTelemetryContext(COPY_TRACE_NAME_PREFIX+x.name, traceLevel, nil)
	...
	copyCtx.SetAttribute(SOURCE_TRACE_ANNOTATION, x.Segment.TraceID)
	return copyCtx
}
