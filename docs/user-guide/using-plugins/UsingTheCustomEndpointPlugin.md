...
2. Add the plugin code `customEndpoint` to the [`wrapperPlugins`](../UsingTheGoWrapper.md#connection-plugin-manager-parameters) value.
...
When using IAM authentication make sure that IAM user has `rds:DescribeDBClusterEndpoints` permission granted. Otherwise, the Custom Endpoint Plugin may not be able to function properly. 