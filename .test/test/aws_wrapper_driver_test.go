...
func setup_getAwsWrapperStmt(ctrl *gomock.Controller, results any) (*mock_driver_infrastructure.MockPluginManager, *awsDriver.AwsWrapperStmt) {
	mockPluginManager := mock_driver_infrastructure.NewMockPluginManager(ctrl)
	mockPluginService := mock_driver_infrastructure.NewMockPluginService(ctrl)
	mockConn := mock_database_sql_driver.NewMockConn(ctrl)
	mockTelemetryFactory := mock_telemetry.NewMockTelemetryFactory(ctrl)
	...
	awsStmt, ok := driverStmt.(*awsDriver.AwsWrapperStmt)
	if !ok {
		return nil, nil
	}

	return mockPluginManager, awsStmt
}
