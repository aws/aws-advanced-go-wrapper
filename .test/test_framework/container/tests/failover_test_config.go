...
type FailoverPluginType int

const (
	FailoverPluginMode FailoverPluginType = iota
	GdbFailoverPluginMode
)

type failoverTestConfig struct {
	name       string
	pluginType FailoverPluginType
	setupFn    func(t *testing.T, env *test_utils.TestEnvironment, host string) map[string]string
}

var failoverConfigs = []failoverTestConfig{
	{
		name:       "Failover",
