...
func efmDisableInstanceConnTest(t *testing.T, cfg failoverTestConfig) {
	...
	defer test_utils.BasicCleanup(t.Name())
	assert.Nil(t, err)
	props := test_utils.GetPropsForProxyWithConnectTimeout(environment, environment.Info().ProxyDatabaseInfo.WriterInstanceEndpoint(), "efm,failover", TEST_FAILURE_DETECTION_INTERVAL_SECONDS)
