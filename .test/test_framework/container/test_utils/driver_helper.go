...
func ExecuteQueryDB(engine DatabaseEngine, db *sql.DB, query string, timeoutValueSeconds int) error {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*time.Duration(timeoutValueSeconds))
	defer cancel()
	...
	for rows.Next() {
		// The values are not needed, only whether reading them fails.
	}
	return rows.Err()
}
