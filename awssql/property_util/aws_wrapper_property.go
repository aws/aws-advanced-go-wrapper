...
var ALL_WRAPPER_PROPERTIES = map[string]bool{
	...
	CUSTOM_ENDPOINT_INFO_REFRESH_RATE_MS.Name:             true,
	CUSTOM_ENDPOINT_INFO_REFRESH_RATE_BACKOFF_FACTOR.Name: true,
	CUSTOM_ENDPOINT_INFO_MAX_REFRESH_RATE_MS.Name:         true,
	CUSTOM_ENDPOINT_ENFORCE_ROLE_FILTERING.Name:           true,
	WAIT_FOR_CUSTOM_ENDPOINT_INFO.Name:                    true,
...
var CUSTOM_ENDPOINT_INFO_REFRESH_RATE_MS = AwsWrapperProperty{
	Name:                "customEndpointInfoRefreshRateMs",
	description:         "Controls how frequently custom endpoint monitors fetch custom endpoint info, in milliseconds.",
	defaultValue:        "30000",
	wrapperPropertyType: WRAPPER_TYPE_INT,
}

var CUSTOM_ENDPOINT_INFO_REFRESH_RATE_BACKOFF_FACTOR = AwsWrapperProperty{
	Name: "customEndpointInfoRefreshRateBackoffFactor",
	description: "Controls the exponential backoff factor for the custom endpoint monitor. When the monitor is " +
		"throttled by the AWS RDS SDK, the time between fetches for custom endpoint info increases by this factor. " +
		"On a successful call it decreases by the same factor.",
	defaultValue:        "2",
	wrapperPropertyType: WRAPPER_TYPE_INT,
}

var CUSTOM_ENDPOINT_INFO_MAX_REFRESH_RATE_MS = AwsWrapperProperty{
	Name: "customEndpointInfoMaxRefreshRateMs",
	description: "Controls the maximum time the custom endpoint monitor will wait between fetches for custom " +
		"endpoint info, in milliseconds.",
	defaultValue:        "300000",
	wrapperPropertyType: WRAPPER_TYPE_INT,
}

var CUSTOM_ENDPOINT_ENFORCE_ROLE_FILTERING = AwsWrapperProperty{
	Name: "customEndpointEnforceRoleFiltering",
	description: "Controls whether a READER-type custom endpoint with an exclusion member list restricts host " +
		"selection to reader instances. Defaults to false so that upgrading does not change routing. This default " +
		"is deprecated and will become true in the next major version.",
	defaultValue:        "false",
	wrapperPropertyType: WRAPPER_TYPE_BOOL,
}

var WAIT_FOR_CUSTOM_ENDPOINT_INFO = AwsWrapperProperty{
	Name: "waitForCustomEndpointInfo",
	description: "Controls whether to wait for custom endpoint info to become available before connecting or executing a " +
...
var WAIT_FOR_CUSTOM_ENDPOINT_INFO_TIMEOUT_MS = AwsWrapperProperty{
	Name: "waitForCustomEndpointInfoTimeoutMs",
	description: "Controls the maximum amount of time that the plugin will wait for custom endpoint info to be made " +
		"available by the custom endpoint monitor, in milliseconds.",
	defaultValue:        "5000",
	wrapperPropertyType: WRAPPER_TYPE_INT,
}

var CUSTOM_ENDPOINT_MONITOR_IDLE_EXPIRATION_MS = AwsWrapperProperty{
	Name:                "customEndpointMonitorExpirationMs",
	description:         "Controls how long a monitor should run without use before expiring and being removed, in milliseconds.",
	defaultValue:        "900000",
	wrapperPropertyType: WRAPPER_TYPE_INT,
}
