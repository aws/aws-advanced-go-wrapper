...
func TestNewOtelContext(t *testing.T) {
	...
	otelCtx0, resultCtx0 := telemetry.NewOpenTelemetryContext(tracer, "test 0", telemetry.NESTED, nil)
	otelCtx1, resultCtx1 := telemetry.NewOpenTelemetryContext(tracer, "test 1", telemetry.NESTED, resultCtx0)
	otelCtx2, resultCtx2 := telemetry.NewOpenTelemetryContext(tracer, "test 2", telemetry.TOP_LEVEL, resultCtx0)
...
func TestOtelClone(t *testing.T) {
	...
	tracer := otel.Tracer("testTracer")
	otelCtx, _ := telemetry.NewOpenTelemetryContext(tracer, "test 0", telemetry.NESTED, nil)
